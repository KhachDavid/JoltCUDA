...
__device__ float atomicMaxFloat(float* addr, float value) {
        ...
        int collidingShapeIndex)
    {
        ...
        if (invMass > 0.f)
        {
            ...
            else
            {
                ...
                {
                    //d_largestPenetration[i] = penetration;
                    float3 normal = (norm_length > 0.f) ? diff / norm_length : make_float3(0.f,0.f,0.f);
                    //d_collisionPlane[i] = norm;
                    //d_collidingShapeIndex[i] = collidingShapeIndex;
                    atomicMaxFloat(&d_largestPenetration[i], penetration);
                    d_collisionPlane[i] = normal;
                    d_collidingShapeIndex[i] = collidingShapeIndex;
                }
        ...
        int collidingShapeIndex)
    {
        ...
        cudaMemset(d_largestPenetration, -FLT_MAX, floatSize);
        ...
        for (int i = 0; i < numVertices; i++) {
            printf("Final largestPenetration[%d]: %f\n", i, hLargestPenetration[i]);
        }
